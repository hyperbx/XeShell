...
$buildPaths = @("XeShell\bin\Publish\")
$patchVersion = ".github\workflows\Patch-Version.ps1"

if ($Help)
{
    echo "XeShell Build Script"
    echo ""
    echo "Usage:"
    echo "-Archive - archives the build artifacts."
    echo "-BuildAll - build XeShell with all available profiles."
    echo "-CommitID [id] - set the commit ID to use from GitHub for the version number."
    echo "-Configuration [name] - build XeShell with a specific configuration."
    echo "-Clean - cleans the solution before building XeShell."
    echo "-Help - display help."
    echo "-Profile [name] - build XeShell with a specific profile."
    echo "-UseFullCommitID - use the full 40 character commit ID for the version number."
    echo "-Version [major].[minor].[revision] - set the version number for this build of XeShell."
    exit
}
...
{
    echo ".NET SDK is required to build XeShell."
